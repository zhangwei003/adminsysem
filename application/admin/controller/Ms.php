...
class Ms extends BaseAdmin
{
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            $this->result($this->logicMs->addMs($this->request->post()));
        }
        //选择渠道
        $channels = $this->logicPay->getChannelList(['status' => 1], true, 'id desc', false);
    ...
    public function edit(Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            $this->assign('channels', $channels);
            //当前码商管理的渠道
            $currentChannel = $this->logicPay->getChannelInfo(['remarks' => $ulist['userid']]);
    ...
    {
        $fields = ['a.*', 'b.username'];
        $map['a.status'] = EwmPayCode::ACTIVE_STATUS;
        $data = $this->logicEwmPayCodes->getCodeList($map, $fields, 'balance desc', false);
        collection($data)->append(['format_acount_number']);
        $count = $this->logicEwmPayCodes->getCodeCount($map);
        $this->result($data || !empty($data) ?
            [
                'code' => CodeEnum::SUCCESS,
