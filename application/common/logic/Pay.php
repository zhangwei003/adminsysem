...
//        $codeInfo = $codeInfo->toArray();
        ...
        $channels = $this->modelPayChannel->getColumn(['id' => ['in', $codeInfo['cnl_id']], 'status' => ['eq', '1']],
            'id,name,action,timeslot,return_url,notify_url,remarks,limit_moneys,pay_address,pay_secret,pay_merchant,channel_fund,deposit,deposit_status');
        ...
        //过滤掉押金状态为开启、渠道资金加当前订单金额大于押金金额的渠道
        foreach ($channels as $d_key => $d_channel){
            if ($d_channel['deposit_status'] == 1 && bccomp(bcadd($d_channel['channel_fund'] , $order['amount'], 2) , $d_channel['deposit']  , 2 )  > 0){
                unset($channels[$d_key]);
            }
        }
        ...
        foreach ($channels as $key => $item1){
            if ($item1['limit_moneys']){
                if ($order['amount'] !=$item1['limit_moneys']){
                    unset($channels[$key]);
                }
            }
        }
        ...
        if (empty($newChannels)) {
            return ['errorCode' => '400028', 'msg' => '金额不存在1'];
        }
        ...
        //规则处理  我先简便写一下
        $channelsMap = [];
        static $channelGroupLimitMoneys = [];
        foreach ($newChannels as $key => $val) {
            ...
            if (1) {
                ...
                $limitMoney = trim($val['limit_moneys']) ? array_filter(explode(',', $val['limit_moneys'])) : [];
                $channelGroupLimitMoneys = array_merge($channelGroupLimitMoneys, $limitMoney);
                $channelsMap[$key] = $val;
            }
        }
        ...
        //如何当前的下单金额没在当前渠道组限制的金额中 -》直接根据渠道组中的全权重去分配
        $channelGroupLimitMoneys = array_unique($channelGroupLimitMoneys);
        $orderAmount = $order['amount'];

        //当前下单金额在渠道中有设置   -----》原来的逻辑
        $map['pay_code_id'] = $codeInfo['co_id'];
        $map['cnl_id'] = ['in', array_keys($channelsMap)];
        $map['price'] = $orderAmount;
        $getchannelsByPrice = Db::name('pay_channel_price_weight')->where($map)->column('cnl_weight', 'cnl_id');

        //查询其他金额，其他统一price字段值为9999
        $map['price'] = 9999;
        $getelsechannelsByPrice = Db::name('pay_channel_price_weight')->where($map)->column('cnl_weight', 'cnl_id');

        if ( (!in_array(intval($orderAmount), [30, 50, 100, 200])  || empty($getchannelsByPrice) )  &&  empty($getelsechannelsByPrice)) {
            //当前下单金额在渠道中没有有设置   -----》原来的逻辑
            ...
            //根据权重值匹配最优秀
            $getchannelsByPrice = $getchannelsByPrice ?: $getelsechannelsByPrice;
            $channelId = roll($getchannelsByPrice);
            $channelId = $channelId == 0 ? array_rand($getchannelsByPrice, 1) : $channelId;
            $channel = $this->modelPayChannel->where(['id' => $channelId])->find()->toArray();
        }
...
from cm_pay_channel pc  where pc.status = 1;');
     ...
     * 生成渠道名称
     */
    protected function createChannelName($msId)
    {
        $prefix = "渠道";
        $ms = $this->modelMs->where(['userid' => (int)$msId])->find();
        return $ms ? $ms['username'] . ": " . $prefix : '';
    }
...
//            if (!isset($data['id'])) {
//                //新增
//                $data['name'] = $this->createChannelName($data['remarks']);
//            }
        ...
        public function AdminSysChannel($data){
        
        $this->modelPayChannel->setInfo($data);

        $action = isset($data['id']) ? '编辑' : '新增';

        action_log($action, '支付渠道' . $data['id']);
     ...
     *
     */
    public function savefastadd($data)
    {
        //TODO 数据验证
        $validate = $this->validateFastadd->check($data);
        

        if (!$validate) {
            return ['code' => CodeEnum::ERROR, 'msg' => $this->validatePayChannel->getError()];
        }
        $data['pay_merchant'] = trim($data['pay_merchant']);
        $data['pay_secret'] = trim($data['pay_secret']);
        //渠道ip白名单处理
        $auth_ips = array_filter(explode(PHP_EOL, $data['notify_ips']));
        $tempIps = [];
        foreach ($auth_ips as $ip) {
        ...
        $data['notify_ips'] = trim(implode(',', $tempIps));
	$contreller = $data['template'];
	if(empty( $data['template']))
	{
	  return ['code' => CodeEnum::ERROR, 'msg' => '系统还未支持该模板，请联系客服'];
	}
	unset( $data['template']);
        if(isset($data['id'])){
            //更新
            db('pay_channel')->where(['id'=>$data['id']])->update($data);
        }else{
            //插入
            $channelId = db('pay_channel')->insertGetId($data);
	    $action = $this->createAction($channelId,$contreller);
	    $notify_url = $_SERVER['HTTP_ORIGIN'].'/api/notify/notify/channel/'.$action;;
            db('pay_channel')->where(['id'=>$channelId])->update(['action'=>$action,'notify_url'=>$notify_url,'return_url'=>'http://www.baidu.com']);
        }
        $action = isset($data['id']) ? '编辑' : '新增';
        action_log($action, '支付渠道' . $data['name']);
       return ['code' => CodeEnum::SUCCESS, 'msg' => $action . '渠道成功'];
    ...
    */
    private function createAction($id,$start){
        ...
        $randNumber = substr ( md5($id.'abccdd'),0,4 );
        $end = 'Pay';
