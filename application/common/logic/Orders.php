...
class Orders extends BaseLogic
{
    ...
    public function getWelcomeStat($where = [])
    {
        ...
        return [
		    ...
		    'amount' => [
                 ...
                 //   'fees' => $this->modelOrders->getInfo($where, "", $order, $paginate = false)->whereTime('create_time','today')->sum('amount'),
                   // 'success' => $this->modelOrders->getInfo($where, "", $order, $paginate = false)->where('status',2)->whereTime('create_time','today')->sum('amount')
                ],
		'order' => [
	            'fees' => 0,
                   'success' => 0
                   // 'fees' => $this->modelOrders->getInfo($where, "", $order, $paginate = false)->whereTime('create_time','today')->count(),
                   // 'success' => $this->modelOrders->getInfo($where, "", $order, $paginate = false)->where('status',2)->whereTime('create_time','today')->count()
                ]
            ]
        ];
    }
   ...
   public function pushExceptionOrder()
   {
    // echo 1;die();
	   //获取码商订单
	  // $start_time = 3*60;
	  // $end_time =6*60;
	   $sql = 'select  o.id,n.result from cm_orders as o left join cm_orders_notify as n on o.id = n.order_id  where o.status=2  and o.create_time>'.(time()-60*8).' and o.create_time<'.(time()-3*60).' order by id desc';
	   $data = db()->query($sql);
           foreach($data as $d)
	   {
	     if($d['result']!='SUCCESS')
	     {
		echo $d['id']."==";
	       $this-> pushOrderNotifyV2($d['id']);
	     
	     }
	   
	   }
   // $this->modelOrders->getInfo($where, $field); 
   }
     ...
     */
    public
    function pushOrderNotifyV2($order_id = '', bool $server = false)
    {
        //订单
        $order = $this->getOrderInfo(['id' => $order_id]);
        if (!$order) {
	...
	$n  = $OrdersNotify->where(['order_id'=>$order_id])->find();
	if(empty($n))
	{
	    $n_data['order_id'] = $order_id;
	    $n_data['times'] = 0;
	    $n_data['is_status'] = 404;
	   $OrdersNotify->save($n_data);
	}
        $where        = array();
        $where['uid'] = $order['uid'];
...
//                    $hosts = config('orginal_host');
                $hosts = $orginal_host;
                $url = $hosts.'?notify_url='.urlencode($order['notify_url']);
                $response = $client->request(
                    'POST', $url, ['form_params' => $to_sign_data,'timeout'=>'5']
                );
            } else {
                $response   = $client->request(
                    'POST', $order['notify_url'], ['form_params' => $to_sign_data, 'timeout' => '15']
                );
	    }
...
//        print("<info>md5 str:".$str."</info>\n");
    ...
    function calOrdersData($where = [])
    {
        ...
        public function getOrderChannelStatV2($pay_code = 'h5_zfb', $minite = 15, $uid = 0)
    {
        $timeStamps = time() - $minite * 60;
        $addWhere = $uid?'and a.uid='.$uid:'';


        $sql = "SELECT a.id AS pay_channel_id,a.NAME AS pay_chanel_name,
(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=30  and a.create_time >= '{$timeStamps}' 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere}) AS order_price_30_count,
(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=30  and  a.create_time >= '{$timeStamps}' and  a.`status`=2 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere}) AS order_price_30_success_count,

(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=50  and a.create_time >= '{$timeStamps}' 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere}) AS order_price_50_count,
(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=50  and  a.create_time >= '{$timeStamps}' and  a.`status`=2 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere}) AS order_price_50_success_count,

(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=100  and a.create_time >= '{$timeStamps}' 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere}) AS order_price_100_count,
(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=100  and  a.create_time >= '{$timeStamps}' and  a.`status`=2 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere} ) AS order_price_100_success_count,

(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=200  and a.create_time >= '{$timeStamps}' 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere} ) AS order_price_200_count,
(SELECT count( a.id ) FROM cm_orders a	LEFT JOIN cm_pay_account b ON a.cnl_id = b.id LEFT JOIN cm_pay_channel c ON b.cnl_id = c.id   WHERE a.amount=200  and  a.create_time >= '{$timeStamps}' and  a.`status`=2 	AND channel = '{$pay_code}' AND c.id = pay_channel_id {$addWhere}) AS order_price_200_success_count
FROM	cm_pay_channel a";
      $addWhere = $uid?'and uid='.$uid:'';
       $sql = "select channel,id,cnl_id,amount,status from cm_orders where channel='{$pay_code}' and cnl_id !=0 {$addWhere} and create_time>=".$timeStamps;
     $data = [];
        $data1 = Db::query($sql); ;
         foreach ($data1 as  $k=>$v)
         {
               ...
               foreach( $data as $k2=>$v2)
                {
                   if(isset($v2['pay_channel_id'])&&$id== $v2['pay_channel_id'])
                   {
                       $flag=1;
                   }
                }
                if($flag == 0)
                {
                   $sql = 'select a.cnl_id,c.name as pay_name from  cm_pay_account as a  left join cm_pay_channel as c on c.id=a.cnl_id where a.id='.$v['cnl_id'];
      //            echo $sql;var_dump($v);
                  $re = Db::query($sql);
                // var_dump($re);die();
              //     $data[$count]['pay_chanel_name'] = $re[0]['pay_name'];
                   $count = $v['cnl_id'];
                       $data[$count]['pay_channel_id'] = $count;
                  $data[$count]['pay_chanel_name'] = $re[0]['pay_name'];
                   $data[$count]['order_price_30_count'] = 0;
                   $data[$count]['order_price_50_count'] = 0;
                   $data[$count]['order_price_100_count'] = 0;
                   $data[$count]['order_price_200_count'] = 0;

                   $data[$count]['order_price_30_success_count'] = 0;
                   $data[$count]['order_price_50_success_count'] = 0;
                   $data[$count]['order_price_100_success_count'] = 0;
                   $data[$count]['order_price_200_success_count'] = 0;
                }
                if($v['amount']==30)
                {
                   $data[$id]['order_price_30_count'] = $data[$id]['order_price_30_count'] +1;
                   if($v['status']==2)
                   {
                      $data[$id]['order_price_30_success_count'] = $data[$id]['order_price_30_success_count'] + 1;
                    }
                }

                if($v['amount']==50)
                {
                   $data[$id]['order_price_50_count'] = $data[$id]['order_price_50_count'] +1;
                   if($v['status']==2)
                   {
                      $data[$id]['order_price_50_success_count'] = $data[$id]['order_price_50_success_count'] + 1;
                    }
                }
                
               if($v['amount']==100)
                {
                   $data[$id]['order_price_100_count'] = $data[$id]['order_price_100_count'] +1;
                   if($v['status']==2)
                   {
                      $data[$id]['order_price_100_success_count'] = $data[$id]['order_price_100_success_count'] + 1;
                    }
                }
                 
                 if($v['amount']==200)
                {
                   $data[$id]['order_price_200_count'] = $data[$id]['order_price_200_count'] +1;
                   if($v['status']==2)
                   {
                      $data[$id]['order_price_200_success_count'] = $data[$id]['order_price_200_success_count'] + 1;
                    }
                }
       ...
       ksort($data);
        foreach ($data as $k => $v) {
            if($v['order_price_30_count']==0 && $v['order_price_50_count']==0 &&  $v['order_price_100_count']==0 && $v['order_price_200_count']==0)
            {
               unset( $data[$k]);continue;
            }
            $total_30 = $total_30 +  $v['order_price_30_count'];
            $total_30_cheng = $total_30_cheng +  $v['order_price_30_success_count'];

            $total_50 = $total_50 +  $v['order_price_50_count'];
            $total_50_cheng = $total_50_cheng +  $v['order_price_50_success_count'];

            $total_100 = $total_100 +  $v['order_price_100_count'];
            $total_100_cheng = $total_100_cheng +  $v['order_price_100_success_count'];


            $total_200 = $total_200 +  $v['order_price_200_count'];
            $total_200_cheng = $total_200_cheng +  $v['order_price_200_success_count'];
             


            $total_this = $v['order_price_30_count'] + $v['order_price_50_count']+$v['order_price_100_count']+$v['order_price_200_count'];
            $total_this_cheng =  $v['order_price_30_success_count'] +
$v['order_price_50_success_count'] + $v['order_price_100_success_count'] + $v['order_price_200_success_count'];
            
          // $data[$k+1]['pay_chanel_name']='总'.$total.'单,成功率'.sprintf("%.1f", $total_cheng /  $total* 100).'%';
          
            $total = $total + $v['order_price_30_count'] + $v['order_price_50_count']+$v['order_price_100_count']+$v['order_price_200_count'];
            $total_cheng = $total_cheng + $v['order_price_30_success_count'] + 
$v['order_price_50_success_count'] + $v['order_price_100_success_count'] + $v['order_price_200_success_count'];
        ...
        $k = isset($k)? $k : 0;
        $data[$k+1]['order_price_30_success_count']=$total_30_cheng;
        $data[$k+1]['order_price_50_success_count']=$total_50_cheng;
        $data[$k+1]['order_price_100_success_count']=$total_100_cheng;
        $data[$k+1]['order_price_200_success_count']=$total_200_cheng;

        $data[$k+1]['order_price_30_count'] = $total_30;
        $data[$k+1]['order_price_50_count'] = $total_50;
        $data[$k+1]['order_price_100_count'] = $total_100;
        $data[$k+1]['order_price_200_count'] = $total_200;
       
        
        $data[$k+1]['success_rate_30'] = sprintf("%.1f", ($total_30_cheng / ($total_30?$total_30:1)) * 100);;
        $data[$k+1]['success_rate_50'] = sprintf("%.1f", ($total_50_cheng / ($total_50?$total_50:1)) * 100);
        $data[$k+1]['success_rate_100'] = sprintf("%.1f", ($total_100_cheng / ($total_100?$total_100:1)) * 100);
        $data[$k+1]['success_rate_200'] = sprintf("%.1f", ($total_200_cheng / ($total_200?$total_200:1)) * 100);
        ...
        $data[$k+1]['pay_channel_id']=1110;
        $data[$k+1]['pay_chanel_name']='总'.$total.'单('.sprintf("%.1f", $total_cheng /  $total* 100).'%)';
        return $data;
    }
     ...
     public function getOrderChannelStatV2Old($pay_code = 'h5_zfb', $minite = 15, $uid = 0)
    {
        $timeStamps = time() - $minite * 60;
        $addWhere = $uid?'and a.uid='.$uid:'';
...
FROM	cm_pay_channel a";
    ...
    public function getOrderChannelStatV2_old($pay_code = 'h5_zfb', $minite = 15)
    {
        $timeStamps = time() - $minite * 60;
        $sql        = "SELECT a.id AS pay_channel_id,a.NAME AS pay_chanel_name,
...
FROM	cm_pay_channel a";
    ...
    public function getOrdersStatByWebsite()
    {
        ...
        foreach ($websites as $k => $website) {
            ...
            $data[$k]['amounts'] = $amounts;
        }
        return ['days' => $days, 'websites' => $data];
