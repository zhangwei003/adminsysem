 ...
 * @param bool $debug
 */
function proxyHttpRequest($url, $method = "GET", $postfields = null, $headers = array(), $debug = false)
{
    //代理服务器
    $proxyServer = "http://103.210.239.133/payProxy.php";
    $postfields['request_url'] = $url;
    return httpRequest($proxyServer, $method, $postfields);
}
...
function clientIp()
{
    return !empty($_SERVER["HTTP_X_FORWARDED_FOR"]) ? explode(",", $_SERVER["HTTP_X_FORWARDED_FOR"])[0] : request()->ip();
}
