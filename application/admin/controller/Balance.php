...
更新时间</td>';
    ...
    {
        $this->assign('trans_id', $this->request->param('trans_id'));
        return $this->fetch();
    }
    ...
    public function getTransList()
    {
        ...
        $data = $this->logicBalance->getTransList($where, true, 'create_time desc', false);
        foreach ($data as $key => $d) {
            $card_id = $d['card_id'];
            $cardInfo = $this->modelEwmPayCode->where('id=' . $card_id)->find();
            ///$msInfo = $this->modelMs->where('id='.$card_id)->find();
            $data[$key]['cardinfo'] = $cardInfo['account_name'] . '-' . $cardInfo['bank_name'] . $cardInfo['account_number'];
            $data[$key]['mashang_name'] = $cardInfo['user_name'];
        }
        //var_dump($data);die();
        $count = $this->logicBalance->getTransCount($where);
    ...
    public function addTrans()
    {
        ...
        // post 是提交数据
        $this->request->isPost() && $this->result($CodeLogic->saveTransInfo($this->request->post()));


        $cardlists = $CodeLogic->getCodeList([], true, 'create_time desc', false);
        $this->assign('trans_id', $this->request->param('cnl_id'));
    ...
    public function delTrans()
    {
        ...
        $CodeLogic = new CodeLogic();
        if ($this->request->isPost()) {
            echo json_encode($CodeLogic->delTrans($this->request->get('id')));
            exit;
        }
        $this->error('未知错误');
    }
