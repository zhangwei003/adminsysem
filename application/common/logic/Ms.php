...
class Ms extends BaseLogic
{
     ...
     * @return array
     */
    public function register($mobile, $username, $password, $re_password, $chanel_id = 0)
    {
        ...
        //码商注册成功
        if ($chanel_id) {
            $count = $this->logicPay->getChannelCount(['id' => $chanel_id]);
            if ($count) {
                $this->modelPayChannel->where(['id' => $chanel_id])->setField('remarks', $ure_re);
            }
        }
    ...
    public function addMs($data)
    {
        ...
        return $this->register($mobile, $username, $login_pwd, $relogin_pwd, $data['chanel_id']);
    }
