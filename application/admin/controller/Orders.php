...
//    }
    ...
    public function subnotify()
    {
        $this->result($this->logicOrders->pushOrderNotifyV2($this->request->param('order_id'), $this->request->param('server')));
    }
    ...
    {
        $this->assign('channel', $this->logicPay->getChannelList());
        $this->assign('payCode', $this->modelPayCode->getPayCodeLists(['status' => 1]));
        return $this->fetch();
    }
     ...
     public function channelStatV2()
    {
        ...
        $minite = $this->request->param('minite', '10');
        $uid = $this->request->param('uid');
        $data = $this->logicOrders->getOrderChannelStatV2($payCode, $minite, $uid);
        $this->result($data || !empty($data) ?
            [
                'code' => CodeEnum::SUCCESS,
                'msg' => '',
     ...
     *
     */
    public function channelStatV2_old()
    {
        $payCode = $this->request->param('pay_code', 'h5_zfb');
     ...
     *
     */
    public function amountcalulate()
    {
        return $this->fetch();
    }
    ...
    public function amountcalulateList()
    {
        ...
        //时间搜索  时间戳搜素
        $date = $this->parseRequestDate3();
        list($start, $end) = $date[1];
        $addWhere = $uid ? 'and uid=' . $uid : '';
        $start = $start?$start:0;
        $sql = "SELECT
	amount,
  COUNT(id) as current_amout_count,
	COUNT(IF(`status`=2,true,NULL))  as  current_amout_success_count,
		COUNT(IF(`status`=1,true,NULL))  as  current_amout_wait_count,
  ROUND(IF(COUNT(IF(`status`=2,true,NULL))=0,0,COUNT(IF(`status`=2,true,NULL))/COUNT(id) ),3) AS current_amout_success_percent,  
	COALESCE(cast(sum(if(status=1,amount,0)) AS decimal(15,2)),0) as current_amout_wait_pay,
	COALESCE(cast(sum(if(status=2,amount,0)) AS decimal(15,2)),0) as current_amout_payd
FROM
	cm_orders 
WHERE
   
	create_time  between  {$start} and {$end} {$addWhere}
GROUP BY
    ...
    public function amountcalulateList2()
    {
