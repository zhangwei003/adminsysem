...
//        echo json_encode($userIds);
    ...
    public function setOrderSucess($orderInfo, $note)
    {
        ...
        $remarks   = $orderInfo['pay_username'] . '-转入';
        $ret       = $CodeLogic->addLogs($orderInfo['code_id'], $orderInfo['order_pay_price'], 1, 1, $remarks, $orderInfo['add_time']);
//var_dump($orderInfo['code_id'],$orderInfo['order_pay_price'],$orderInfo['pay_username']);die();
    ...
    /**
     * 获取相同卡号在最近十分钟相同金额的订单数目
     *
     */
    public function recentSameMoneyAndSameCardOrderCount($curentOrderId = 0)
    {
        $limitSeconeds            = 10 * 60;
        $GemapayOrderModel        = new \app\common\model\EwmOrder();
        $order                    = $GemapayOrderModel->where(['id' => $curentOrderId])->find();
        $where['add_time']        = ['between', [$order['add_time'] - $limitSeconeds, $order['add_time'] + $limitSeconeds]];
        $where['order_pay_price'] = $order['order_pay_price'];
        $where['code_id']         = $order['code_id'];
        $where['status']          = EwmOrder::STATUS_YES;
        $where['pay_username']    = $order['pay_username'];
        return $GemapayOrderModel->where($where)->count();
    }
