...
class PayChannelChange extends BaseLogic
{
     ...
     *api修改渠道鉴权
     */
    public function channelChangeForAdminAuth($authData = [])
    {
        $validate = $this->validatePayChannel->check($authData);
        if (!$validate) {
            return ['code' => CodeEnum::ERROR, 'msg' => $this->validatePayChannel->getError()];
        }
        $admin = (new Admin())->where(['username' => $authData['login_name']])->find();
        if (empty($admin)) {
            return ['code' => CodeEnum::ERROR, 'msg' => "授权账号{$authData['login_name']}不存在"];
        }
        if (data_md5_key($authData['login_pwd']) != $admin['password']) {
            return ['code' => CodeEnum::ERROR, 'msg' => "授权账号{$authData['login_name']}密码[login_pwd]错误"];
        }
